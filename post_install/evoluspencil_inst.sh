...
while true; do
    ...
    case $answer in
        [Yy]* )
            echo "Then download the newer version and install it with sudo dpkg -i *.deb. Also don't forget to change the version in this post_install script. As a last step you could also check, whether they finally provide a symlink."
            read -p "Press any key to continue... " -n1 -s
            break;; 
        [Nn]* )
            echo "Allright! Then we just install the last version automatically =)"
            read -p "Press any key to continue... " -n1 -s
            wget http://evoluspencil.googlecode.com/files/evoluspencil_2.0.5_all.deb; sudo dpkg -i evoluspencil_2.0.5_all.deb
            break;; 
        * )
